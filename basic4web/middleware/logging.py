...
class CustomLogger(logging.Logger):
    ...
    def warn(self, msg, *args, **kwargs):
        ...
        filename = os.path.basename(frame.f_globals.get("__file__", ""))
        lineno = frame.f_lineno
        super().warning(f"[{filename}][{caller_method}][{lineno}] {msg}", *args, **kwargs)
...
formatter = logging.Formatter(f"%(asctime)s - {base_config.get('CORE_VERSION')} - %(levelname)s - %(message)s")
